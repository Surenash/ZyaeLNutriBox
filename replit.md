...
**Health Goals / Meal Plans Section**: Unified section displaying meal plans from API in compact horizontal card layout with small thumbnail images (96px) on left, content on right. Features title, description, pricing (original/discounted), star ratings, review counts, optional badges, and "Subscribe Now" buttons. Grid layout: grid-cols-1 md:grid-cols-2 for responsive 2-column design. Clicking any card opens detailed modal showing full description, key benefits, nutritional information (calories, protein, carbs, fats), sample daily meals, and what's included. Real-time data fetched from `/api/meal-plans` endpoint with loading skeletons. API data automatically extended with default benefits/meals/macros for modal display.
