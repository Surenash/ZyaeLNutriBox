...
export default function LiveChat({ roomId, onClose }: LiveChatProps) {
  ...
  useEffect(() => {
    if (isConnected) {
      // Join chat room
      emit('join_chat_room', { roomId, userId: user?.id });
    }
  ...
  const handleTyping = (text: string) => {
    ...
    if (text.trim() && !isTyping) {
      setIsTyping(true);
      emit('typing', { roomId, userId: user?.id, userName: user?.name, isTyping: true });
    } else if (!text.trim() && isTyping) {
      setIsTyping(false);
      emit('typing', { roomId, userId: user?.id, userName: user?.name, isTyping: false });
    }
  };
