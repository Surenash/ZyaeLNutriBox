...
export default function PlansScreen() {
  ...
  const { data: mealPlans = plans, isLoading } = useQuery({
    queryKey: ['meal-plans'],
    queryFn: async () => {
  ...
  const handleSelectPlan = (planId: number) => {
    ...
    Alert.alert(
      ...
      [
        ...
        { text: 'Subscribe', onPress: () => subscribeMutation.mutate(planId) },
      ]
    );
  };
  ...
  const renderPlan = ({ item }: { item: any }) => (
    <Card style={styles.planCard}>
      ...
      <Button
        ...
        disabled={!!currentSubscription || subscribeMutation.isPending}
        loading={subscribeMutation.isPending && selectedPlan === item.id}
