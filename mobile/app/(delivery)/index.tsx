...
import { View, Text, StyleSheet, ScrollView, FlatList, Alert, ActivityIndicator, Linking } from 'react-native';
...
interface DeliveryOrder {
  ...
  mealType: string;
  quantity?: number;
  status: 'assigned' | 'picked_up' | 'picked-up' | 'in_transit' | 'in-transit' | 'delivered';
    ...
    queryFn: async () => {
      try {
        ...
        return (response.data || []).map((order: any) => {
          const status = (order.status || 'assigned').replace('-', '_') as DeliveryOrder['status'];
          
          // Parse coordinates from current_location or address
          let coords = { latitude: 0, longitude: 0 };
          if (order.currentLocation) {
            coords = {
              latitude: parseFloat(order.currentLocation.latitude) || 0,
              longitude: parseFloat(order.currentLocation.longitude) || 0,
            };
          }
          
          // Parse estimated delivery time
          const estTime = order.estimatedDeliveryTime 
          ...
          return {
            ...
            orderNumber: order.orderId ? order.orderId.substring(0, 8).toUpperCase() : 'N/A',
            clientName: order.clientName || 'Unknown',
            clientPhone: order.clientPhone || '',
            clientAddress: order.clientAddress || '',
            mealType: order.mealType || 'Unknown',
            quantity: order.quantity || 1,
            status,
            priority: (order.priority || 'medium') as 'low' | 'medium' | 'high',
            estimatedDeliveryTime: estTime,
            distance: 0, // Can be calculated from coordinates if available
            deliveryFee: 50, // Default fee, can be from order if available
        ...
        try {
          ...
          return (response.data || []).map((order: any) => ({
            ...
            mealType: order.mealType || 'Unknown',
            quantity: order.quantity || 1,
            status: (order.status || 'assigned').replace('-', '_') as DeliveryOrder['status'],
            priority: (order.priority || 'medium') as 'low' | 'medium' | 'high',
          ...
          Alert.alert('Error', 'Failed to load delivery orders. Please try again.');
          return [];
        }
      }
    },
  ...
  const { mutate: updateStatusMutation, isPending } = useMutation({
    mutationFn: async ({ orderId, status }: { orderId: string; status: string }) => {
      try {
        ...
        // Update order status
        await apiClient.put(`/api/orders/${orderId}/status`, { status: apiStatus });
      ...
      <View style={styles.actionButtons}>
        ...
        {item.coordinates && item.coordinates.latitude !== 0 && (
          <Button
            ...
            onPress={() => {
              ...
              Linking.openURL(url).catch(err => Alert.alert('Error', 'Could not open navigation'));
            }}
            size="small"
            variant="outline"
          />
        ...
        <Button
          ...
          onPress={() => {
            ...
            Linking.openURL(phoneUrl).catch(err => Alert.alert('Error', 'Could not make call'));
          }}
          size="small"
          variant="outline"
        />
